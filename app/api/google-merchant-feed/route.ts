...
function getFullImageUrl(imageUrl: string | null): string {
  if (!imageUrl) {
    return `${process.env.NEXT_PUBLIC_SITE_URL || 'https://hashtag34stories.com'}/images/products/placeholder.png`;
  }
  ...
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://hashtag34stories.com';
...
export async function GET() {
  try {
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://hashtag34stories.com';
    ...
    xml += `  <channel>\n`;
    xml += `    <title>Hashtag34 Stories Products</title>\n`;
    xml += `    <link>${siteUrl}</link>\n`;
    xml += `    <description>Hashtag34 Stories - Custom T-Shirts &amp; Corporate Gifting Products</description>\n`;
    ...
    products?.forEach((product) => {
      ...
      xml += `      <g:brand>Hashtag34 Stories</g:brand>\n`;
